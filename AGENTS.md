...
. Package names with hyphens (`ripgrep-all`) are stored as `tri_p_ripgrep_all`, `tri_f_ripgrep_all`, `tri_b_ripgrep_all` — the conversion happens in both the Edge Functions and `kv-sync.py`.
